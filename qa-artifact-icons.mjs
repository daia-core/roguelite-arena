...
// Pull {id, name} pairs in roster order. Artifacts now render via their own
// hand-crafted `art_<id>` glyph through getArtifactIcon(id), not an emoji.
const ARTS = [...src.matchAll(/id:\s*'([^']*)',\s*name:\s*(?:'([^']*)'|"([^"]*)")/g)]
  .map(m => ({ id: m[1], name: m[2] || m[3] }));
...
const result = await page.evaluate((arts) => {
  const getIcon = window.__getArtifactIcon;
  if (!getIcon) return { error: 'window.__getArtifactIcon not exposed' };
  const out = [];
  for (const a of arts) {
    try {
      const s = getIcon(a.id);
      ...
      // >=3 colors ensures a real hand-crafted glyph (outline + ramp), not a flat rune fallback.
      out.push({ name: a.name, id: a.id, ok: painted > 8 && colors.size >= 3, painted, colors: colors.size });
    } catch (err) { out.push({ name: a.name, id: a.id, ok: false, reason: String(err) }); }
  }
  return { out };
}, ARTS);
...
if (result.error) { console.log('ERROR:', result.error); fail = true; }
  ...
  if (!r.ok) fail = true;
  console.log(`  [${tag}] ${r.name.padEnd(20)} ${r.reason || `${r.painted}px ${r.colors}c`}`);
}
...
await page.evaluate((arts) => {
  ...
  for (const a of arts) {
    const s = getIcon(a.id); if (!s) continue;
