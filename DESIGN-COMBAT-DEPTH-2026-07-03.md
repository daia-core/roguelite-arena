...
- [x] Phase 4b — **element-keyed projectile sprites** (shipped 2026-07-25): `Projectile.ts` has `damageType: DamageType` field + `setElement()` + `ELEMENT_COLORS`; `sprites.ts` registers `bullet_fire/ice/lightning/poison`; `Player.ts` calls `getShotElement()` + `p.setElement(element)` for every projectile. Element-tinted bullets are live.
...
**Deployed live (2026-07-03, evening):** Phases 1 + 2 + 3a + 3b + 4 + 4b + 5 all shipped to Vercel
production (alias `roguelite-game-blush.vercel.app`). Remaining: Phase 3c (item-uniqueness review — Felix-gated).
