...
const result = await page.evaluate(() => {
  ...
  const expected = (s) => {
    ...
    return {
      ...
      getMaxHealth: Math.max(1, s.baseMaxHealth + sum('maxHealthBonus') + tf.maxHealthBonus + du.maxHealthBonus + s.artifactMaxHealthBonus),
      ...
      getCritMultiplier: Math.min(s.constructor.SANITY_MULT_CAP, critKnee(
        s.baseCritMultiplier * prod('critDamageMultiplier') * tf.critDamageMultiplier * du.critDamageMultiplier
        * s.artifactCritMultMult * s.skillCritMultMult)),
      getHealthRegen: sum('healthRegen') + du.healthRegen,
      getArmor: s.metaArmor + sum('armor') + tf.armor + du.armor,
      ...
      getDodgeChance: Math.min(s.constructor.DODGE_CAP, sum('dodge') + du.dodge),
      ...
      getShopDiscount: Math.min(0.3, sum('shopDiscount') + tf.shopDiscount + du.shopDiscount),
      getInterestBonus: Math.min(0.4, sum('interestBonus')),
      getLuck: Math.min(1.0, sum('luck')),
      hasExplosionOnHit: any('explosionOnHit') || du.explosionOnHit,
