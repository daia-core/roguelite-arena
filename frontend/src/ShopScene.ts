  ...
  private drawStatsPopup(): void {
    ...
    const groups: Array<[string, string, Row[]]> = [
      ['OFFENSE', '#ffa94d', [
        ...
        ['Melee Eff Dmg', num(ps.getMeleeDamage()), ps.getMeleeDamageMult() > 1.001],
        ['Ranged Dmg', mult(ps.getRangedDamageMult()), ps.getRangedDamageMult() > 1.001],
        ['Ranged Eff Dmg', num(ps.getRangedDamage()), ps.getRangedDamageMult() > 1.001],
      ...
      ['SPECIAL', '#e599f7', [
        ...
        ['Burn', pct(ps.getBurnChance()), ps.getBurnChance() > 0],
        ['Bleed', pct(ps.getBleedChance()), ps.getBleedChance() > 0],
        ['Doom', pct(ps.getDoomChance()), ps.getDoomChance() > 0],
        ['Wound', pct(ps.getWoundChance()), ps.getWoundChance() > 0],
        ['Slow', pct(ps.getSlowStrength()), ps.getSlowStrength() > 0],
        ['Fragility', pct(ps.getFragileChance()), ps.getFragileChance() > 0],
        ['Exposed', pct(ps.getExposedChance()), ps.getExposedChance() > 0],
        ['Brittle', pct(ps.getBrittleChance()), ps.getBrittleChance() > 0],
        ['Condemned', pct(ps.getCondemnedChance()), ps.getCondemnedChance() > 0],
        ['Dazed', pct(ps.getDazedChance()), ps.getDazedChance() > 0],
        ['Disoriented', pct(ps.getDisorientedChance()), ps.getDisorientedChance() > 0],
        ['Poison Spread', 'YES', ps.hasPoisonSpread()],
        ['Burn Spread', 'YES', ps.hasBurnSpread()],
        ['Multicast', pct(ps.getMulticastChance()), ps.getMulticastChance() > 0],
