...
- ✅ **7. Roster audit (broken-build review)** — **done 2026-07-02 late night.** Ran `tools/qa/_audit-items.mjs` over the whole `ItemDatabase`: found + fixed a duplicate-id bug (two different "Leech Blade" items both `leech_blade_t3` — one shadowed the other). Repurposed the strictly-dominated legacy one into **Siphon Rounds** (`siphon_rounds_t3`, +15% lifesteal / +20% ranged / −15% fire rate) — the first ranged-lifesteal item, enabling a vampiric-sniper build. Confirmed no other degenerate stacks: dodge hard-capped at 75%, all other stat sums bounded/intended. Build clean; qa-damagetype + qa-builddiv PASS.
